...
describe('lessonTopicsService', () => {
  it('Should get get lesson topics by lesson id', async () => {
    sinon.stub(LessonTopicsModel, 'find').returns({
      exec: sinon.stub().resolves([{ topics: { tokens: lessonTopicsMock[0].tokens } }])
    })
  ...
  it('Should get get lesson topics by lesson id', async () => {
    sinon.stub(tokenService, 'mapTokenStatus').returns(lessonTopicsMock[0].tokens)
    sinon.stub(LessonTopicsModel, 'aggregate').returns([{ topics: { tokens: lessonTopicsMock[0].tokens } }])
    const lessonTopics = await lessonTopicsService.getLessonTopicsByIdAndTopic('5ea3c3580675cc60fc8a275e', 0)
  ...
  it('Should delete lesson topic ', async () => {
    const mongoStub = sinon.stub(LessonTopicsModel, 'findOneAndUpdate').returns()
  ...
  it('Should update lesson topic', async () => {
    ...
    await lessonTopicsService.updateLessonTopic('5ea3c3580675cc60fc8a275e', lessonTopicsMock[0])
    expect(mongoStub.calledOnce).to.be.true
