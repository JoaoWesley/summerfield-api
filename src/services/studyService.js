  ...
  await StudyModel.findOneAndUpdate(
    { user: 'admin@gmail.com', 'items.wordPhrase': new RegExp(item.wordPhrase, 'i') },
    {
      $set: {
        ...
        ...(item.lastReviewedDate ? { 'items.$.lastReviewedDate': item.lastReviewedDate } : {}),
        ...(item.nextReviewDate ? { 'items.$.nextReviewDate': item.nextReviewDate } : {}),      
      },
      ...(item.lessonIds ? 
...
export const getItemsToReview = async (lessonId) => {  
  ...
  items.sort( (a, b) => new Date(a.nextReviewDate).getTime() - new Date(b.nextReviewDate).getTime())

  const reviewedInLast24hours = items.filter( (item) => {
    const diffInHoursLastReviewed = moment().diff(moment(item.lastReviewedDate), 'hours', true)
  ...
  if(lessonId) {
    ...
    maxNumberItemsReview = items.length < maxNumberItemsReview ? items.length : maxNumberItemsReview

    let numberOfItemsToReview = maxNumberItemsReview - reviewedInLast24hours.length 
    numberOfItemsToReview = numberOfItemsToReview > 0 ? numberOfItemsToReview : 0
...
export const evaluate = async (item) => {
  let { evaluation }  = item  
  item.evaluation = supermemo2(evaluation.quality, evaluation.schedule, evaluation.factor)   

  item.lastReviewedDate = moment().format('YYYY-MM-DD HH:mm')
  item.nextReviewDate = moment().add(item.evaluation.schedule, 'day').format('YYYY-MM-DD HH:mm')
